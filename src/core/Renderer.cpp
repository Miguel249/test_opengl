...
Renderer::Renderer() : snakeHeadTexture(0), cellTexture(0) {}

Renderer::~Renderer() {
    if (snakeHeadTexture) glDeleteTextures(1, &snakeHeadTexture);
    if (cellTexture) glDeleteTextures(1, &cellTexture);
}
...
void Renderer::renderSnake(const Snake& snake, const Grid& grid) const {
    ...
    for (size_t i = 0; i < body.size(); ++i) {
        ...
        if (isHead) {
            ...
            switch (dir) {
                case Direction::UP:    dirVec = glm::vec2(0, 1); break;
                case Direction::DOWN:  dirVec = glm::vec2(0, -1); break;
                case Direction::LEFT:  dirVec = glm::vec2(-1, 0); break;
                case Direction::RIGHT: dirVec = glm::vec2(1, 0); break;
                default: break;
            }
